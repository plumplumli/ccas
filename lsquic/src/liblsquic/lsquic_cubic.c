...
//    LSQ_ERROR("cwnd is %"PRIu64" bytes", cubic->cu_cwnd);
}
                                            ...
                                            enum quic_ft_bit UNUSED_retx_frames)
{
    ...
    cubic->cu_rtt_stats = &conn_pub->rtt_stats; //lzj: forever pointer
...
//    printf("When acked, cubic cwnd is:%lld\n", cubic->cu_cwnd);
...
static void
lsquic_cubic_loss (void *cong_ctl, lsquic_time_t largest_lost_sent_time)
{
    struct lsquic_cubic *const cubic = cong_ctl;
...
//    LSQ_ERROR("loss detected, last_max_cwnd: %lu, cwnd: %lu",
//             cubic->cu_last_max_cwnd, cubic->cu_cwnd);
//    lsquic_time_t sRTT = cubic->cu_rtt_stats->srtt;
//    lsquic_time_t minRTT = cubic->cu_rtt_stats->min_rtt;
//    unsigned cubic_bw = cubic->cu_cwnd * 1000000 / sRTT;
...
//        pacing_rate = bandwidth + bandwidth / 5;
    else
        pacing_rate = bandwidth + bandwidth / 4;
//        pacing_rate = bandwidth + bandwidth / 5;
//        pacing_rate = bandwidth / 2;

//    LSQ_ERROR("bandwidth is %"PRIu64" Bps, ""pacing_rate is %"PRIu64" Bps, ""cwnd is %"PRIu64" bytes",
//              bandwidth,
//              pacing_rate,
//              cubic->cu_cwnd);

//    pacing_rate = ~(uint64_t)0;
