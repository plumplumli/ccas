...
get_target_cwnd (const struct lsquic_bbr *bbr, float gain)
{
    ...
    bdp = get_min_rtt(bbr) * BW_TO_BYTES_PER_SEC(&bw) / 1000000; //lzj: usec->sec, bps->Bps

//    LSQ_ERROR("srtt is %"PRIu64" usec, ""min_rtt is %"PRIu64" usec, ""bdp is %"PRIu64" bytes",
//              bbr->bbr_rtt_stats->srtt,
//              bbr->bbr_rtt_stats->min_rtt,
//              bdp);
...
//        LSQ_ERROR("pacing_rate is %"PRIu64" bps, ""cwnd is %"PRIu64" bytes",
//                  0,
//                  cwnd);
//        printf("in probr_rtt\n");
    }
    ...
    if (bbr->bbr_mode == BBR_MODE_PROBE_RTT)
    {
        ...
        else
        {
                        ...
                        && (bbr->bbr_flags & BBR_FLAG_PROBE_RTT_ROUND_PASSED))
            {
                ...
                // lzj: bbr has finished probe_rtt, now we're ready to switch to copa
                bbr->ready_to_copa = 1;
            }
        }
    }
...
lsquic_bbr_end_ack (void *cong_ctl, uint64_t in_flight)
{
    ...
    // lzj: bbr has is not ready to switch to copa
    bbr->ready_to_copa = 0;
...
//    LSQ_ERROR("pacing_rate is %"PRIu64" bps, ""cwnd is %"PRIu64" bytes",
//          bbr->bbr_pacing_rate.value,
//          bbr->bbr_cwnd);

//    printf("pacing rate is %lld Bps, cwnd is %lld bytes\n", bbr->bbr_pacing_rate.value, bbr->bbr_cwnd);

//    LSQ_ERROR("bbr_cwnd is %"PRIu64" bytes, ""bbr_min_cwnd is %"PRIu64" bytes""bbr_recovery_window is %"PRIu64" bytes, ",
//              bbr->bbr_cwnd,
//              bbr->bbr_min_cwnd,
//              bbr->bbr_recovery_window);
...
static void
lsquic_bbr_loss (void *cong_ctl, lsquic_time_t largest_lost_sent_time)
{
    struct lsquic_bbr *const bbr = cong_ctl;
//    LSQ_ERROR("loss detected, cwnd: %lu", bbr->bbr_cwnd);

//    printf("BBR LOSS\n");

    if (bbr->loss_flag == 0) {
        bbr->loss_srtt = bbr->bbr_rtt_stats->srtt;
        bbr->loss_flag = 1;
//        printf("bbr->loss_srtt is: %lld us\n", bbr->loss_srtt);
//        printf("current bw is: %d Mbps\n", bbr->bbr_cwnd * 1000000 / bbr->loss_srtt / 1024 / 1024);
    }
//    lsquic_time_t sRTT = bbr->bbr_rtt_stats->srtt;
//    lsquic_time_t minRTT = bbr->bbr_rtt_stats->min_rtt;
//    unsigned bbr_bw = bbr->bbr_cwnd * 1000000 / sRTT;
//    printf("bbr_bw is: %lld bps\n", bbr_bw);
//    unsigned bbr_buffer_size = (sRTT - minRTT) * 750 * 1000000 / 1000000 / 8;
//    printf("bbr_buffer_size is: %lld bps\n", bbr_buffer_size);
}
