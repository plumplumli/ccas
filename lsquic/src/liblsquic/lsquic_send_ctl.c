...
send_ctl_select_cc_smooth (struct lsquic_send_ctl *ctl, int acc_flags)
{
    ...
    if (acc->acc_flags == ACC_BBR && acc_flags == ACC_COPA) {
        ...
        if (acc->acc_bbr.ready_to_copa == 1) {
            ...
            printf("succeesfully bbr to cubic smoothly\n");
            return;
        }
    }

    /* wait until probe rtt finish, and then switch to Copa */
    // 1. google
    // 2.
...
//    buffer_size_by_bw_2 = (sRTT - minRTT) * bw_2 / 1000000 / 8; //B
    printf("buffer_size_by_bw_2 is: %lld Byte\n", buffer_size_by_bw_2);
...
//            ctl->sc_flags |= SC_CLEANUP_BBR;
                ...
                printf("After loss, CUBIC cwnd is: %lld\n", ctl->sc_adaptive_cc.acc_cubic.cu_cwnd);
                printf("\n");
            }
        }
...
//            send_ctl_select_cc(ctl);
        ...
        /* LZJ：每隔15s切换一次算法 */
        if ((now - created) / (15 * 1000000) > ctl->circle && ctl->sc_ci == &lsquic_cong_adaptive_if) {
