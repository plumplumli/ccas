...
#include "lsquic_full_connection_ietf.h" //LZJ: used to get begin time
//#include "lsquic_enc_sess.h" //LZJ: used to get fc
#include "lsquic_enc_sess_ietf.h" //LZJ: used to get fc
          ...
          struct lsquic_conn_public *conn_pub, enum send_ctl_flags flags)
{
    ...
    {
    case 0:
        ctl->sc_ci = &lsquic_cong_copa_if;
        ctl->sc_cong_ctl = &ctl->sc_adaptive_cc.acc_copa;
        break;
    case 1:
...
send_ctl_transfer_time (void *ctx)
{
    ...
    struct ietf_full_conn *conn = (struct ietf_full_conn *)(ctl->sc_conn_pub->lconn);
    lsquic_time_t created = conn->ifc_created;
    lsquic_time_t now = lsquic_time_now();
    printf("time is %lld, pacing_rate is %lld\n", (now - created), pacing_rate);

//    if (pacing_rate == ~(uint64_t)0)
//        return 0;
                             ...
                             struct lsquic_packet_out *packet_out)
{
    ...
    ctl->size_total_sent += packet_out->po_data_sz; //lzj: for Copa, used to get total sent size from communication start
...
static void
send_ctl_select_cc_smooth (struct lsquic_send_ctl *ctl, int acc_flags)
{
    struct adaptive_cc *const acc = CGP(ctl);

    /* switch smoothly from BBR to CUBIC */
    if (acc->acc_flags == ACC_BBR && acc_flags == ACC_CUBIC) {
        acc->acc_flags = ACC_CUBIC;
        acc->acc_cubic.cu_cwnd = acc->acc_bbr.bbr_cwnd;
        ctl->sc_ci->cci_loss(CGP(ctl), ctl->largest_lost_sent_time);
        return;
    }

    /* switch smoothly from BBR to Copa */
    if (acc->acc_flags == ACC_BBR && acc_flags == ACC_COPA) {
        // lzj: push BBR to enter probe_rtt mode
        acc->acc_bbr.bbr_mode = BBR_MODE_PROBE_RTT;

        // maybe_enter_or_exit_probe_rtt()
        // 1. it would set CWND during PROBE_RTT is kMinimumCongestionWindow
        // 2. it has signal which tell us whether probe_rtt has finished
        // 3. it would be called at cci_end_ack()
        // so we check flag (bbr.ready_to_copa = 1) after cci_end_ack()

        // lzj: bbr has finished rtt probe, it's time to switch to Copa
        if (acc->acc_bbr.ready_to_copa == 1) {
//            acc->acc_copa.cwnd_bytes = acc->acc_bbr.bbr_cwnd;
            acc->acc_flags = ACC_COPA;
            printf("succeesfully bbr to copa smoothly\n");
            return;
        }
    }
...
//    if (acc->acc_flags == ACC_BBR && acc_flags == ACC_COPA) {
//        acc->acc_flags = ACC_COPA;
//        printf("succeesfully bbr to copa roughly\n");
//        return;
//    }
...
static void
send_ctl_select_cc_rough (struct lsquic_send_ctl *ctl, int acc_flags)
{
    struct adaptive_cc *const acc = CGP(ctl);

    /* switch roughly from BBR to CUBIC */
//    if (acc->acc_flags == ACC_BBR && acc_flags == ACC_CUBIC) {
//        acc->acc_flags = ACC_CUBIC;
//        return;
//    }

    /* switch roughly from BBR to Copa */
    if (acc->acc_flags == ACC_BBR && acc_flags == ACC_COPA) {
        acc->acc_flags = ACC_COPA;
        acc->acc_copa.cwnd_bytes = acc->acc_bbr.bbr_cwnd;
        printf("succeesfully bbr to copa roughly\n");
...
static void
send_ctl_select_cc_multi (struct lsquic_send_ctl *ctl, int acc_flags)
{
    struct adaptive_cc *const acc = CGP(ctl);

    acc->acc_flags = acc_flags;
}
...
{
    /*
    // desgin 1 (lsquic original) --- according first rtt
    lsquic_time_t srtt;
    ...
    */
    ...
    /*
    // design 2 (lzj 1.0) --- according connections number
    unsigned n_conns;
    ...
    */
    ...
    /* LZJ: 一次切换，根据buffer、bw、minRTT、FC四个指标，选择算法 */
    unsigned fc = 0; //记录flow control参数
    lsquic_time_t minRTT = 0; //记录最小RTT
    lsquic_time_t sRTT = 0; //记录平滑RTT
    unsigned bw = 0;
    unsigned bw_1 = 0;
    unsigned bw_2 = 0;
    unsigned pacing_rate = 0;
    unsigned buffer_size_by_bw_2 = 0;
    unsigned buffer_size_by_pacing_rate = 0;

//    unsigned bw_cubic = 0;

//    struct ietf_mini_conn *conn = (struct ietf_mini_conn *) lconn; //示例
    struct ietf_mini_conn *imc = (struct ietf_mini_conn *)(ctl->sc_conn_pub->lconn);
    struct enc_sess_iquic *const enc_sess = (struct enc_sess_iquic *)(imc->imc_conn.cn_enc_session);
    fc = enc_sess->esi_peer_tp.tp_numerics[3];
    printf("Receiver's Connection level Flow Control Window is: %lld\n", fc);

    struct adaptive_cc *const acc = CGP(ctl);
    bw = acc->acc_bbr.bbr_max_bandwidth.samples[0].value; //探测bandwidth, bps
//    printf("bw is: %lld bps\n", bw);
    bw_1 = acc->acc_bbr.bbr_max_bandwidth.samples[1].value; //探测bandwidth, bps
//    printf("bw_1 is: %lld bps\n", bw_1);
    bw_2 = acc->acc_bbr.bbr_max_bandwidth.samples[2].value; //探测bandwidth, bps
    printf("btlbw is: %lld bps\n", bw_2);
    pacing_rate = acc->acc_bbr.bbr_pacing_rate.value; //pacing_rate, bps
//    printf("pacing_rate is: %lld bps\n", pacing_rate);
    minRTT = ctl->sc_conn_pub->rtt_stats.min_rtt; //微妙, 最小RTT
//    minRTT = acc->acc_bbr.bbr_min_rtt; //微妙
    printf("minRTT is: %lld us\n", minRTT);
    sRTT = ctl->sc_conn_pub->rtt_stats.srtt; //微妙
//    sRTT = acc->acc_bbr.bbr_rtt_stats->srtt; //微妙
    printf("sRTT is: %lld us\n", sRTT);
    if (acc->acc_bbr.loss_flag == 1) buffer_size_by_bw_2 = (acc->acc_bbr.loss_srtt - minRTT) * bw_2 / 1000000 / 8; //B
    else buffer_size_by_bw_2 = (sRTT - minRTT) * bw_2 / 1000000 / 8; //B
//    buffer_size_by_bw_2 = (sRTT - minRTT) * bw_2 / 1000000 / 8; //B
    printf("btlbuf is: %lld Byte\n", buffer_size_by_bw_2);
    printf("loss_flag is: %d\n", acc->acc_bbr.loss_flag);
//    buffer_size_by_pacing_rate = (sRTT - minRTT) * pacing_rate / 1000000 / 8; //B
//    printf("buffer_size_by_pacing_rate is: %lld Byte\n", buffer_size_by_pacing_rate);

//    unsigned cubic_bw = acc->acc_cubic.cu_cwnd * 1000000 / sRTT;
//    printf("cubic_bw is: %lld bps\n", cubic_bw);
//    unsigned cubic_buffer_size = (sRTT - minRTT) * cubic_bw / 1000000 / 8;
//    printf("cubic_buffer_size is: %lld bps\n", cubic_buffer_size);

//    printf("Copa pacing_rate is: %lld bps\n", acc->acc_copa.pacing_rate);
//    printf("sRTT is: %lld us\n", sRTT);
//    if (acc->acc_bbr.loss_flag == 1) {
//        printf("buffer_size_by_copa is: %lld Byte\n", (acc->acc_bbr.loss_srtt - minRTT) * acc->acc_copa.pacing_rate / 1000000 / 8);
//    }
//    else {
//        printf("buffer_size_by_copa is: %lld Byte\n", (sRTT - minRTT) * acc->acc_copa.pacing_rate / 1000000 / 8);
//    }
//
//    printf("\n");

    if (fc > 5242880) { //无流量控制限制
        if (acc->acc_bbr.loss_flag == 1 && buffer_size_by_bw_2 < 1000 * 1000) { //浅缓冲区:前5s发生丢包, 且测量缓冲区不大
            printf("Buffer: Shallow; Flow Control: No;\n");
            if (bw_2 < (20+50)/2 * 1024 * 1024 || //区域一: [10,20]Mbps
                    (bw_2 > (20+50)/2 * 1024 * 1024 && bw_2 < (50+100)/2 * 1024 * 1024 && minRTT < 75 * 1000) ||
                    (bw_2 > (50+100)/2 * 1024 * 1024 && bw_2 < (100+250)/2 * 1024 * 1024 && minRTT < 50 * 1000) ||
                    (bw_2 > (100+250)/2 * 1024 * 1024 && bw_2 < (500+750)/2 * 1024 * 1024 && minRTT < 10 * 1000)) {
                printf("Now select Cubic congestion controller\n");
                printf("CUBIC cwnd is: %lld bytes\n", ctl->sc_adaptive_cc.acc_cubic.cu_cwnd);
                printf("BBR cwnd is: %lld bytes\n", ctl->sc_adaptive_cc.acc_bbr.bbr_cwnd);
//                ctl->sc_ci = &lsquic_cong_cubic_if;
//                ctl->sc_cong_ctl = &ctl->sc_adaptive_cc.acc_cubic;
//            ctl->sc_flags |= SC_CLEANUP_BBR;
                acc->acc_flags = ACC_CUBIC;
                acc->acc_cubic.cu_cwnd = acc->acc_bbr.bbr_cwnd; //同步bbr cwnd
                ctl->sc_ci->cci_loss(CGP(ctl), ctl->largest_lost_sent_time); //提前认为发生丢包
                printf("Smoothly change CUBIC cwnd: %lld bytes\n", ctl->sc_adaptive_cc.acc_cubic.cu_cwnd);
//                printf("\n");
            }
        }

        if ((acc->acc_bbr.loss_flag == 1 && buffer_size_by_bw_2 >= 1000 * 1000) || acc->acc_bbr.loss_flag == 0) {  //深缓冲区:前5s发生丢包, 且测量缓冲区不大;或前5s没有丢包
            printf("Buffer: Deep; Flow Control: No;\n");
            if (acc->acc_bbr.loss_flag == 1) {
                if ((50 * 1000 <= minRTT && minRTT < 200 * 1000 && bw_2 > (50+100)/2 * 1024 * 1024)) {
                    printf("Now select Cubic congestion controller\n");
                    printf("CUBIC cwnd is: %lld bytes\n", ctl->sc_adaptive_cc.acc_cubic.cu_cwnd);
                    printf("BBR cwnd is: %lld bytes\n", ctl->sc_adaptive_cc.acc_bbr.bbr_cwnd);
//                    ctl->sc_ci = &lsquic_cong_cubic_if;
...
//                    printf("\n");
                ...
                if ((5 * 1000 <= minRTT && minRTT < 50 * 1000 && bw_2 > (250+500)/2 * 1024 * 1024 && bw_2 < (750+1000)/2 * 1024 * 1024)) {
                    printf("Now select Cubic congestion controller\n");
                    printf("CUBIC cwnd is: %lld bytes\n", ctl->sc_adaptive_cc.acc_cubic.cu_cwnd);
                    printf("BBR cwnd is: %lld bytes\n", ctl->sc_adaptive_cc.acc_bbr.bbr_cwnd);
//                    ctl->sc_ci = &lsquic_cong_cubic_if;
...
//            ctl->sc_flags |= SC_CLEANUP_BBR;
                    ...
                    acc->acc_cubic.cu_cwnd = acc->acc_bbr.bbr_cwnd; //同步bbr cwnd
                    ctl->sc_ci->cci_loss(CGP(ctl), ctl->largest_lost_sent_time); //提前认为发生丢包
                    printf("After loss, CUBIC cwnd is: %lld bytes\n", ctl->sc_adaptive_cc.acc_cubic.cu_cwnd);
//                    printf("\n");
                }
            }
        }
        ...
        if (acc->acc_bbr.loss_flag == 1 && buffer_size_by_bw_2 < 1000 * 1000) { //浅缓冲区:前5s发生丢包, 且测量缓冲区不大
            printf("Buffer: Shallow; Flow Control: Yes;\n");
            if (0 ||
                (bw_2 > 10 * 1024 * 1024 && bw_2 < 20 * 1024 * 1024 && minRTT < 75 * 1000) ||
                (bw_2 > 20 * 1024 * 1024 && bw_2 < 100 * 1024 * 1024 && minRTT > 100 * 1000)) {
                printf("Now select Cubic congestion controller\n");
                printf("CUBIC cwnd is: %lld bytes\n", ctl->sc_adaptive_cc.acc_cubic.cu_cwnd);
                printf("BBR cwnd is: %lld bytes\n", ctl->sc_adaptive_cc.acc_bbr.bbr_cwnd);
//                ctl->sc_ci = &lsquic_cong_cubic_if;
...
//    if (buffer_size_by_bw_2 < 1000 * 1000 && fc > 5242880) { //浅缓冲区, 无流量控制限制
//        printf("Buffer: Shallow; Flow Control: No;\n");
//        if (bw < (20+50)/2 * 1024 * 1024 ||
//            (bw > (20+50)/2 * 1024 * 1024 && bw < (100+250)/2 * 1024 * 1024 && minRTT < 50 * 1000) ) {
//            printf("Now select Cubic congestion controller\n");
//            printf("CUBIC cwnd is: %lld\n", ctl->sc_adaptive_cc.acc_cubic.cu_cwnd);
//            printf("BBR cwnd is: %lld\n", ctl->sc_adaptive_cc.acc_bbr.bbr_cwnd);
//            ctl->sc_ci = &lsquic_cong_cubic_if;
//            ctl->sc_cong_ctl = &ctl->sc_adaptive_cc.acc_cubic;
////            ctl->sc_flags |= SC_CLEANUP_BBR;
//            acc->acc_cubic.cu_cwnd = acc->acc_bbr.bbr_cwnd; //同步bbr cwnd
//            ctl->sc_ci->cci_loss(CGP(ctl)); //提前认为发生丢包
//            printf("CUBIC reinit cwnd is: %lld\n", ctl->sc_adaptive_cc.acc_cubic.cu_cwnd);
//        }
//    }
//    else if (buffer_size > 500 * 1000 && fc > 5242880) { //深缓冲区, 无流量控制限制
//        printf("Buffer: Deep; Flow Control: No;\n");
//        if ((bw > (500+750)/2 * 1024 * 1024 && bw < (750+1000)/2 * 1024 * 1024 && minRTT < 200 * 1000) ||
//                (10 * 1000 < minRTT && minRTT < 200 * 1000)) {
//            printf("Now select CUBIC congestion controller\n");
//            printf("CUBIC cwnd is: %lld\n", ctl->sc_adaptive_cc.acc_cubic.cu_cwnd);
//            printf("BBR cwnd is: %lld\n", ctl->sc_adaptive_cc.acc_bbr.bbr_cwnd);
//            ctl->sc_ci = &lsquic_cong_cubic_if;
//            ctl->sc_cong_ctl = &ctl->sc_adaptive_cc.acc_cubic;
////    ctl->sc_ci->cci_reinit(CGP(ctl)); //方法一：reinit cubic
//            acc->acc_cubic.cu_cwnd = acc->acc_bbr.bbr_cwnd; //方法二：同步bbr cwnd
////            acc->acc_cubic.cu_epoch_start = 0; //quiet处理
//            ctl->sc_ci->cci_loss(CGP(ctl)); //方法三: 提前认为发生丢包
//            printf("CUBIC reinit cwnd is: %lld\n", ctl->sc_adaptive_cc.acc_cubic.cu_cwnd);
//        }
//    }
//    else if (buffer_size < 1000 * 1000 && fc <= 5242880) {
//        printf("Buffer: Shallow; Flow Control: Yes;\n");
//        if ((bw > 500 * 1024 * 1024 && bw < 750 * 1024 * 1024 && minRTT < 25 * 1000) ||
//                || ) {
//            printf("Now select CUBIC congestion controller\n");
//            printf("CUBIC cwnd is: %lld\n", ctl->sc_adaptive_cc.acc_cubic.cu_cwnd);
//            printf("BBR cwnd is: %lld\n", ctl->sc_adaptive_cc.acc_bbr.bbr_cwnd);
                 ...
                 lsquic_time_t now, lsquic_time_t lack_delta)
{
    ...
    if ((!packno || packno > ctl->sc_max_rtt_packno) && lack_delta < measured_rtt)
    {
        ...
        //printf("srtt is %lld\n", ctl->sc_conn_pub->rtt_stats.srtt);
...
//            send_ctl_select_cc(ctl);
        ...
        printf("time is %lld, srtt is %lld\n", (now - created), ctl->sc_conn_pub->rtt_stats.srtt);
        printf("time is %lld, current rtt is %lld\n", (now - created), measured_rtt);
//        printf("time is %lld, srtt is %lld\n", (now - created), ctl->sc_conn_pub->rtt_stats.min_rtt);

        /* LZJ：每隔25s切换一次算法 */
        if ((now - created) / (25 * 1000000) > ctl->circle && ctl->sc_ci == &lsquic_cong_adaptive_if) {
            //printf("ietf_full_conn->ifc_created is: %lld\n", created);
            //printf("now is: %lld\n", now);
            ctl->circle++;
//            send_ctl_select_cc_multi(ctl, (ctl->circle % 3) + 1);
        }
        /* LZJ: 5s后根据网络状态切换一次算法 */
//        if ((now - created) > (5 * 1000000) && ctl->circle == 0 && ctl->sc_ci == &lsquic_cong_adaptive_if) {
//            printf("ietf_full_conn->ifc_created is: %lld\n", created);
//            printf("now is: %lld\n", now);
//            ctl->circle++;
//            send_ctl_select_cc(ctl);
//        }
//        if ((now - created) > (60 * 1000000) && ctl->srtt_flag == 0) {
//            lsquic_time_t sRTT = ctl->sc_conn_pub->rtt_stats.srtt; //微妙
//            ctl->srtt_flag = ctl->srtt_flag + 1;
//            printf("60s, sRTT is: %lld us\n", sRTT);
//        }
    }

//    LSQ_ERROR("srtt is %"PRIu64" usec, ""min_rtt is %"PRIu64" usec" ,
//              ctl->sc_conn_pub->rtt_stats.srtt,
//              ctl->sc_conn_pub->rtt_stats.min_rtt);
}
...
{
//    printf("lzj check SIGSEGV, point 1, for loss\n");
    ctl->sc_ci->cci_loss(CGP(ctl), ctl->largest_lost_sent_time);
//    printf("lzj check SIGSEGV, point 2, for loss\n");
    if (ctl->sc_flags & SC_PACE)
...
//    lsquic_time_t largest_lost_sent_time = 0;
                ...
                packet_out = next)
    {
        ...
        //lzj: For Copa, take down loss time
        if (packet_out->po_sent > ctl->largest_lost_sent_time)
            ctl->largest_lost_sent_time = packet_out->po_sent;
                         ...
                         lsquic_time_t ack_recv_time, lsquic_time_t now)
{
    ...
    ctl->size_total_sent_prior_ack = ctl->size_total_sent_curr_ack; // total sent data size when receiving current ACK
    ctl->size_total_sent_curr_ack = ctl->size_total_sent; // total sent data size when receiving last ACK
  ...
  after_checks:
            ...
            /* lzj: new ack callback */
            // time when receiving current ACK
//            ctl->size_total_sent_prior_ack = ctl->size_total_sent_curr_ack; // total sent data size when receiving current ACK
//            ctl->size_total_sent_curr_ack = ctl->size_total_sent; // total sent data size when receiving last ACK
            //            printf("lzj check SIGSEGV, point 1, for ack\n");
            ctl->sc_ci->cci_ack(CGP(ctl), packet_out, packet_sz, now,
                                app_limited, ack_recv_time, ctl->size_total_sent_prior_ack, ctl->size_total_sent_curr_ack);
//            printf("lzj check SIGSEGV, point 2, for ack\n");
//            ctl->sc_ci->cci_ack(CGP(ctl), packet_out, packet_sz, now,
//                                                             app_limited);
...
//    printf("srtt is %lld\n", ctl->sc_conn_pub->rtt_stats.srtt);
    ...
    /* maybe it can switch */
    if (ctl->circle > 0) { //SP is OK
        send_ctl_select_cc_smooth(ctl, ACC_COPA);
//        send_ctl_select_cc_rough(ctl, ACC_COPA);
    }
