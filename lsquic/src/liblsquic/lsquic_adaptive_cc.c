...
#define CALL_BOTH(method, ...) do {                             \
    ...
    lsquic_cong_copa_if.method(&acc->acc_copa, __VA_ARGS__);    \
} while (0)
...
#define CALL_BOTH0(method) do {                                 \
    ...
    lsquic_cong_copa_if.method(&acc->acc_copa);                 \
} while (0)
                 ...
                 lsquic_time_t recv_time_cur_ack, uint64_t size_total_sent_prior_ack, uint64_t size_total_sent_curr_ack)
{
    ...
    CALL_BOTH(cci_ack, packet_out, packet_sz, now, app_limited, recv_time_cur_ack, size_total_sent_prior_ack, size_total_sent_curr_ack);
}


static void
adaptive_cc_loss (void *cong_ctl, lsquic_time_t largest_lost_sent_time)
{
    struct adaptive_cc *const acc = cong_ctl;

//    CALL_BOTH0(cci_loss);
    CALL_BOTH(cci_loss, largest_lost_sent_time);
}
...
adaptive_cc_get_cwnd (void *cong_ctl)
{
    ...
    rv[2] = lsquic_cong_copa_if.cci_get_cwnd(&acc->acc_copa);
...
//    acc->acc_flags = 1;
    ...
    rv[2] = lsquic_cong_copa_if.cci_pacing_rate(&acc->acc_copa, in_recovery);
...
//    return rv[2];
    ...
    switch (acc->acc_flags) {
        case ACC_CUBIC:
            printf("cubic pacing_rate is %lld bps\n", rv[0]);
            return rv[0];
            break;
        case ACC_BBR:
            printf("bbr pacing_rate is %lld bps\n", rv[1]);
            return rv[1];
            break;
        case ACC_COPA:
