...
typedef struct lsquic_send_ctl {
    ...
    int                             circle; //LZJ: used to switch CCA every 5s
    int                             srtt_flag; //LZJ: used to record sRTT when 60s
    uint64_t                        size_total_sent; //LZJ: for Copa, used to get total sent size from communication start
    uint64_t                        size_total_sent_prior_ack; //LZJ: for Copa, get total sent size when prior ack
    uint64_t                        size_total_sent_curr_ack; //LZJ: for Copa, get total sent size when current ack
    lsquic_time_t                   largest_lost_sent_time; //LZJ: for Copa, get latest loss packet's sent_time
} lsquic_send_ctl_t;
